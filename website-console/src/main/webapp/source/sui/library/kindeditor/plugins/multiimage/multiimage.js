...
K.extend(KSWFUpload, {
	init : function(options) {
		...
		self.div = K(options.container).html([
			...
			'</div>',
			'<div class="ke-inline-block ke-swfupload-desc" style="color:red">' + options.uploadDesc + '</div>',
...
//				serverData = serverData.replace("[","").replace("]","");
				...
				file.url = data[0].url;
				K('.ke-img', itemDiv).attr('src', file.url).attr('data-status', file.filestatus).data('data', data[0]);
				K('.ke-status > div', itemDiv).hide();
			}
		};
...
//		uploadJson = K.undef(self.uploadJson, self.basePath + 'php/upload_json.php'),
		uploadJson = K.undef(self.uploadJson, $("#APP_ROOT_PATH").val() + '/store/edit_upload_pic'),
		imgPath = self.pluginsPath + 'multiimage/images/',
		imageSizeLimit = K.undef(self.imageSizeLimit, '2MB'),
		imageFileTypes = K.undef(self.imageFileTypes, '*.jpg;*.gif;*.png'),
		imageUploadLimit = K.undef(self.imageUploadLimit, 20),
//		filePostName = K.undef(self.filePostName, 'imgFile'),
		filePostName = K.undef(self.filePostName, 'pic'),
		lang = self.lang(name + '.');
